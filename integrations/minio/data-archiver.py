...
class DataArchiver:
    ...
    def __init__(self, config_path: str):
        """
        Initializes a DataArchiver instance with the specified configuration file.
        
        Loads configuration settings and prepares internal state for connecting to RabbitMQ,
        MinIO, and managing message batches.
        """
    ...
    def _load_config(self, config_path: str) -> Dict:
        ...
        Reads the specified YAML file, parses its contents, and verifies that required sections ('rabbitmq', 'minio', 'archival') are present. Exits the application if loading or validation fails.
    ...
    async def setup(self):
        """
        Initializes connections to RabbitMQ and MinIO, and configures MinIO buckets for archiving.
        
        Raises:
            Exception: If connection to RabbitMQ or MinIO fails.
        """
    ...
    async def _setup_buckets(self):
        """
        Creates required MinIO buckets and applies lifecycle policies for retention and tiering.
        
        For each archive configuration, ensures the corresponding bucket exists in MinIO and sets up lifecycle rules for data expiration and storage class transitions if specified.
        """
    ...
    def _get_object_path(self, queue_name: str) -> str: # Removed timestamp argument
        ...
        The path is structured as: queue_name/YYYY/MM/DD/HH/messages_<timestamp>.json.zst, where <timestamp> is the current UTC epoch time in seconds.
        """
    ...
    async def _archive_batch(self, queue_name: str, batch: MessageBatch):
        ...
        Compresses the batch using zstandard, attaches metadata, and uploads the result to the configured MinIO bucket. Retries the upload on failure according to configuration. Updates Prometheus metrics for archived messages and storage usage. Clears the batch upon successful archival.
        """
    ...
    async def _process_message(self, message: aio_pika.IncomingMessage):
        """
        Processes a single RabbitMQ message, batching it for archival and handling acknowledgment.
        
        Parses the message body as JSON and adds it to a batch for the corresponding queue. If the batch reaches its configured size or age, triggers archival to object storage. Messages without a matching archive configuration are acknowledged and dropped. Non-JSON messages are rejected without requeue, and other processing errors result in a negative acknowledgment without requeue.
        """
    ...
    async def run(self):
        """
        Runs the main event loop for the data archiver service.
        
        Initializes the service, starts the Prometheus metrics server, consumes messages from configured RabbitMQ queues, periodically checks and archives message batches, and handles graceful shutdown on errors or exit signals.
        """
        self.loop = asyncio.get_running_loop()
        try:
            await self.setup()
...
def main():
    """
    Parses command-line arguments, initializes the DataArchiver, and starts the archiving service.
